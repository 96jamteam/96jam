...
public:
	...
	~ResourceManager()
	{
		for(typename ResourceList::iterator it = myResources.begin(), end = myResources.end(); it != end; it++)
		{
	...
	Resource<Type, Identifier, MaxNum> Get(const Identifier &id)
	{
		...
		typename ReferenceList::iterator it = myReferences.find(id);
...
private:
	...
	void *GetMemory()
	{
		...
		{
			typename ResourceList::iterator resourceEnd = myResources.end();
			for(typename ReferenceList::iterator it = myReferences.begin(), end = myReferences.end(); it != end; it++)
			{
				typename ResourceList::iterator resourceIterator = myResources.find(it->first);
